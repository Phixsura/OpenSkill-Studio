...
4. **Capability roll-up** — `requires_capabilities` across chained packs' latest release manifests, deduplicated by **`(capability, frozenset(features))`** — NOT unioned per capability (two steps needing `feat_a` and `feat_b` separately must not demand one offering superset of both; the runtime resolves per step — ADR-011/R83-R84), checked against org offerings (ADR-011); unsatisfied → gaps with `NO_ELIGIBLE_PROVIDER`. Never auto-connects.
5. Recommended skill packs from chained manifests attach as optional items (family `skill_pack`, `required: false`). A recommended slug that resolves to **no published/visible pack** becomes a visible gap `{code: RECOMMENDED_PACK_UNAVAILABLE, slug}` — never a silent drop (every omission is a first-class row).
...
**No self-attestation (R86).** Because `approved_submission` and `commercial_project` are weight-1.0 platform-verified evidence keyed on the *actor* (the submission author / brief applicant), and their capability derivation runs through free text that snake-cases to a capability key, a single actor who is both producer and approver could mint fabricated "verified" evidence with no second party — flipping a hard S2 exclusion into a ranked shortlist entry. Both approval paths therefore forbid self-review: `create_review` rejects a reviewer who is the submission author (`SELF_REVIEW_FORBIDDEN` 403, mirroring the peer-review module's self-exclusion), and `review_application` rejects an applicant accepting/rejecting their own application (403; self-*withdraw* stays allowed). Evidence can only be minted by a distinct second party.
