...
- Field names smuggled into non-sensitive `config` are rejected (`CREDENTIAL_IN_CONFIG` 422); fields the adapter did not declare are rejected (`UNKNOWN_CREDENTIAL_FIELD` 422). The same smuggling vector via a workflow **definition's step `config`** is closed structurally: every step-config schema is `extra="forbid"` (ADR-010, `WF_CONFIG_INVALID`) — an `api_key` key in a `provider_action` config never persists into the definition (which the anon registry preview and run snapshots serve verbatim).
