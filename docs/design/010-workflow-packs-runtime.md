...
| `WF_CONFIG_INVALID` | Per-step-type config schema violation (discriminated union). Config schemas are `extra="forbid"` — an unknown key is rejected, not silently persisted. This is a **credential red-line guard**: without it, an author could smuggle `api_key`/`secret`/`token` fields into a step's `config`, and the definition (persisted verbatim, served in the anon registry preview and run snapshots) would carry the secret through every serialization surface ADR-011 forbids |
...
- **provider_action** steps write `provider_request_id` + `offering_id` + a lease **and commit before the outbound call** (write-ahead idempotency, R13). After that commit the executor re-reads the step status (a concurrent cancel may have flipped it the instant the row lock released) and bails before spending provider money; the read transaction is closed before the outbound call so no connection sits idle-in-transaction for the call's duration. Credentials are decrypted only at this point (ADR-011). Timeout via `workflow_step_timeout_seconds`; failures retry up to `max_attempts` (output cleared on re-claim). Every settlement UPDATE carries the claimed attempt as a **fencing token** — a resurrected executor cannot settle a newer attempt. **Crash-retry pins the recorded offering (R85):** once the write-ahead has persisted `provider_request_id` + `offering_id`, a retry executes on THAT offering or hard-stops `BINDING_STALE` — it never re-runs the binding ladder. The reused idempotency key only dedupes within a single provider account; re-resolution could land on a cheaper offering on a *different* connection added between attempts and re-charge work the crashed attempt already paid for, while erasing the audit record of the account actually charged.
...
3. Extraction only: custom node class_types (vs a vendored 73-node core snapshot), model file references (`.safetensors/.ckpt/...`) with `whitelist`/`structural` confidence labels, input/output nodes, detected capabilities (14-entry `NODE_CAPABILITY_MAP`). Node classification (core vs custom, input/output, capability) is keyed on the **raw** `class_type`, never the sanitized form (R86): `sanitize_untrusted_text` runs NFKC, which folds a crafted fullwidth `ＫSampler` to the core name `KSampler` — classifying on the sanitized value counted a disguised custom node as trusted core and erased it from the custom-node dependency warning. Sanitize is applied only to the stored/displayed label; `core_count` is derived from the raw-based custom tally so a fold-disguise cannot inflate it.
4. Byte-exact original stored with sha256 in `comfyui_imports` (immutable provenance); node titles pass through `sanitize_untrusted_text`. The parsed import is screened for NUL **and non-finite floats** (`NaN`/`Infinity`/`-Infinity`) before the `original_json` JSONB write (R86) — `json.loads` accepts the bare tokens and yields real floats that Postgres rejects (22P02) at insert; both are a clean 422, never a 500.
