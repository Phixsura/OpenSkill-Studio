...
async def test_unicode_cohort_name_cjk(c):
    ...
    r = await c.post(f"/api/v1/orgs/{oid}/cohorts", json={
        "name": "秋季AI设计班 2026",
    }, headers=h)
...
async def test_concurrent_cohort_join_max_learners(c):
    ...
    cid = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={
        "name": "Limited", "max_learners": 1,
...
async def test_concurrent_duplicate_submission(c):
    ...
    pid = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "Conc Sub", "description": "d" * 10, "instructions": "i" * 10,
...
async def test_expired_token_rejected(c):
    ...
    # Use a garbage token
    r = await c.get("/api/v1/orgs", headers={
...
async def test_delete_cohort_with_members_and_assignments(c):
    ...
    cid = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "Delete Me"}, headers=h)).json()["data"]["id"]
    await c.post(f"/api/v1/orgs/{oid}/cohorts/{cid}/members", json={"user_id": us["id"]}, headers=h)

    cat = (await c.post(f"/api/v1/orgs/{oid}/categories", json={"name": "DC"}, headers=h)).json()["data"]["id"]
    sk = (await c.post(f"/api/v1/orgs/{oid}/skills", json={
        "name": "DS", "description": "d" * 10, "difficulty": "beginner", "category_id": cat,
    }, headers=h)).json()["data"]["id"]
...
async def test_student_cannot_access_other_students_drilldown(c):
    ...
    oid = await _org(c, h)
    await c.post(f"/api/v1/orgs/{oid}/members", json={"user_id": us_a["id"], "role": "student"}, headers=h)
    await c.post(f"/api/v1/orgs/{oid}/members", json={"user_id": us_b["id"], "role": "student"}, headers=h)

    cid = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "Auth"}, headers=h)).json()["data"]["id"]
    await c.put(f"/api/v1/orgs/{oid}/cohorts/{cid}", json={"status": "active"}, headers=h)
    await c.post(f"/api/v1/orgs/{oid}/cohorts/{cid}/members", json={"user_id": us_a["id"]}, headers=h)
    await c.post(f"/api/v1/orgs/{oid}/cohorts/{cid}/members", json={"user_id": us_b["id"]}, headers=h)
...
async def test_cross_org_cohort_access_rejected(c):
    ...
    cid = (await c.post(f"/api/v1/orgs/{oid2}/cohorts", json={"name": "Private"}, headers=h2)).json()["data"]["id"]
...
async def test_student_cannot_assign_skills(c):
    ...
    cid = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "Perm"}, headers=h)).json()["data"]["id"]
    cat = (await c.post(f"/api/v1/orgs/{oid}/categories", json={"name": "PC"}, headers=h)).json()["data"]["id"]
    sk = (await c.post(f"/api/v1/orgs/{oid}/skills", json={
        "name": "PS", "description": "d" * 10, "difficulty": "beginner", "category_id": cat,
    }, headers=h)).json()["data"]["id"]
