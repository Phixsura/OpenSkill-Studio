...
async def _org(c, h):
    r = await c.post("/api/v1/orgs", json={"name": f"CRUD-{uuid.uuid4().hex[:6]}"}, headers=h)
...
async def test_project_full_lifecycle(c):
    ...
    # Update
    r = await c.put(f"/api/v1/orgs/{oid}/projects/{pid}", json={
...
async def test_project_deliverable_crud(c):
    ...
    pid = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "Deliv Test", "description": "d", "instructions": "i",
        "rubric": [{"criterion": "Q", "max_score": 100}],
    }, headers=h)).json()["data"]["id"]

    # Create deliverable
    r = await c.post(f"/api/v1/orgs/{oid}/projects/{pid}/deliverables", json={
    ...
    # Update deliverable
    r = await c.put(f"/api/v1/orgs/{oid}/projects/{pid}/deliverables/{did}", json={
        "name": "Updated Hero",
    }, headers=h)
    assert r.status_code == 200
...
async def test_submission_create_submit_detail(c):
    ...
    pid = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "Sub Test", "description": "d", "instructions": "i",
        "rubric": [{"criterion": "Q", "max_score": 100}],
    }, headers=h)).json()["data"]["id"]
...
async def test_skill_full_lifecycle(c):
    ...
    # Update
    r = await c.put(f"/api/v1/orgs/{oid}/skills/{sk_id}", json={
...
async def test_skill_exercise_crud(c):
    ...
    h, _ = await _auth(c)
    oid = await _org(c, h)
    cat = (await c.post(f"/api/v1/orgs/{oid}/categories", json={"name": "Ex"}, headers=h)).json()["data"]["id"]
    sk = (await c.post(f"/api/v1/orgs/{oid}/skills", json={
        "name": "ExSkill", "description": "d" * 10, "difficulty": "beginner", "category_id": cat,
    }, headers=h)).json()["data"]["id"]

    # Create exercise
    r = await c.post(f"/api/v1/orgs/{oid}/skills/{sk}/exercises", json={
        "title": "Practice 1",
    ...
    # Update exercise
    r = await c.put(f"/api/v1/orgs/{oid}/exercises/{ex_id}", json={
...
async def test_evaluation_settings_crud(c):
    ...
    # Enable
    r = await c.put(f"/api/v1/orgs/{oid}/settings/evaluation", json={
...
async def test_evaluation_usage_empty(c):
    ...
    h, _ = await _auth(c)
    oid = await _org(c, h)
    await c.put(f"/api/v1/orgs/{oid}/settings/evaluation", json={
        "enabled": True, "monthly_budget_usd": 100,
    }, headers=h)
...
async def test_evaluation_trigger_disabled(c):
    ...
    pid = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "Eval Test", "description": "d", "instructions": "i",
        "rubric": [{"criterion": "Q", "max_score": 100}],
    }, headers=h)).json()["data"]["id"]
    await c.post(f"/api/v1/orgs/{oid}/projects/{pid}/publish", headers=h)
    ...
    r = await c.post(f"/api/v1/orgs/{oid}/evaluation/trigger", json={
        "submission_id": sid, "type": "submission_review",
    }, headers=h)
...
async def test_cohort_progress_mixed_statuses(c):
    ...
    oid = await _org(c, h)
    await c.post(f"/api/v1/orgs/{oid}/members", json={"user_id": us1["id"], "role": "student"}, headers=h)
    await c.post(f"/api/v1/orgs/{oid}/members", json={"user_id": us2["id"], "role": "student"}, headers=h)

    cid = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "Mix"}, headers=h)).json()["data"]["id"]
    await c.put(f"/api/v1/orgs/{oid}/cohorts/{cid}", json={"status": "active"}, headers=h)
    await c.post(f"/api/v1/orgs/{oid}/cohorts/{cid}/members", json={"user_id": us1["id"]}, headers=h)
    await c.post(f"/api/v1/orgs/{oid}/cohorts/{cid}/members", json={"user_id": us2["id"]}, headers=h)

    pid = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "Mix Proj", "description": "d" * 10, "instructions": "i" * 10,
        "rubric": [{"criterion": "Q", "max_score": 100}],
    }, headers=h)).json()["data"]["id"]
    await c.post(f"/api/v1/orgs/{oid}/projects/{pid}/publish", headers=h)
    ...
    # Student 1 submits
    sid = (await c.post(f"/api/v1/orgs/{oid}/projects/{pid}/submissions", headers=hs1)).json()["data"]["id"]
...
async def test_convert_brief_default_title_and_rubric(c):
    ...
    r = await c.post(f"/api/v1/orgs/{oid}/briefs/{bid}/convert", json={
...
async def test_visibility_three_way_or_filter(c):
    ...
    p1 = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "OrgWide", "description": "d" * 10, "instructions": "i" * 10,
        "rubric": [{"criterion": "Q", "max_score": 100}],
    }, headers=h)).json()["data"]["id"]
    await c.post(f"/api/v1/orgs/{oid}/projects/{p1}/publish", headers=h)

    # Cohort-assigned project
    cid = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "Vis"}, headers=h)).json()["data"]["id"]
    await c.put(f"/api/v1/orgs/{oid}/cohorts/{cid}", json={"status": "active"}, headers=h)
    await c.post(f"/api/v1/orgs/{oid}/cohorts/{cid}/members", json={"user_id": us["id"]}, headers=h)
    p2 = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "CohortOnly", "description": "d" * 10, "instructions": "i" * 10,
        "rubric": [{"criterion": "Q", "max_score": 100}],
    }, headers=h)).json()["data"]["id"]
...
async def test_bulk_enroll_partial_success(c):
    ...
    cid = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "Bulk"}, headers=h)).json()["data"]["id"]

    r = await c.post(f"/api/v1/orgs/{oid}/cohorts/{cid}/members/bulk", json={
...
async def test_delete_project_cascades_from_cohort(c):
    ...
    pid = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "Cascade", "description": "d" * 10, "instructions": "i" * 10,
        "rubric": [{"criterion": "Q", "max_score": 100}],
    }, headers=h)).json()["data"]["id"]

    c1 = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "C1"}, headers=h)).json()["data"]["id"]
    c2 = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "C2"}, headers=h)).json()["data"]["id"]
...
async def test_delete_cohort_doesnt_delete_project(c):
    ...
    pid = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "Survivor", "description": "d" * 10, "instructions": "i" * 10,
        "rubric": [{"criterion": "Q", "max_score": 100}],
    }, headers=h)).json()["data"]["id"]

    cid = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "Doomed"}, headers=h)).json()["data"]["id"]
    await c.post(f"/api/v1/orgs/{oid}/cohorts/{cid}/projects", json={"project_id": pid}, headers=h)
...
async def test_remove_member_keeps_submissions(c):
    ...
    cid = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "Keep"}, headers=h)).json()["data"]["id"]
    await c.put(f"/api/v1/orgs/{oid}/cohorts/{cid}", json={"status": "active"}, headers=h)
    await c.post(f"/api/v1/orgs/{oid}/cohorts/{cid}/members", json={"user_id": us["id"]}, headers=h)

    pid = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "Keep Sub", "description": "d" * 10, "instructions": "i" * 10,
        "rubric": [{"criterion": "Q", "max_score": 100}],
    }, headers=h)).json()["data"]["id"]
    await c.post(f"/api/v1/orgs/{oid}/projects/{pid}/publish", headers=h)
    sid = (await c.post(f"/api/v1/orgs/{oid}/projects/{pid}/submissions", headers=hs)).json()["data"]["id"]
...
async def test_multi_cohort_member_sees_projects_from_each(c):
    ...
    c1 = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "MC1"}, headers=h)).json()["data"]["id"]
    c2 = (await c.post(f"/api/v1/orgs/{oid}/cohorts", json={"name": "MC2"}, headers=h)).json()["data"]["id"]
    for cid in [c1, c2]:
        await c.put(f"/api/v1/orgs/{oid}/cohorts/{cid}", json={"status": "active"}, headers=h)
        await c.post(f"/api/v1/orgs/{oid}/cohorts/{cid}/members", json={"user_id": us["id"]}, headers=h)

    p1 = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "From C1", "description": "d" * 10, "instructions": "i" * 10,
        "rubric": [{"criterion": "Q", "max_score": 100}],
    }, headers=h)).json()["data"]["id"]
    await c.post(f"/api/v1/orgs/{oid}/projects/{p1}/publish", headers=h)
    ...
    p2 = (await c.post(f"/api/v1/orgs/{oid}/projects", json={
        "title": "From C2", "description": "d" * 10, "instructions": "i" * 10,
        "rubric": [{"criterion": "Q", "max_score": 100}],
    }, headers=h)).json()["data"]["id"]
    await c.post(f"/api/v1/orgs/{oid}/projects/{p2}/publish", headers=h)
