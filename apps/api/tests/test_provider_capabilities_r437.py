...
async def _org(db):
    ...
    owner = User(email=f"r437-{uuid.uuid4().hex[:10]}@t.com",
                 password_hash=hash_password("Test123!"), display_name="R437",
                 role=UserRole.ADMIN, status=UserStatus.ACTIVE)
    db.add(owner)
    await db.flush()
    o = await OrgService(db).create(name=f"R437 {uuid.uuid4().hex[:5]}",
...
async def _offering(db, org_id, capability, features, *, conn_status="active", active=True):
    ...
    db.add(conn)
    await db.flush()
    off = ProviderModelOffering(connection_id=conn.id, capability_key=capability,
                               model_name="m", features=features, is_active=active)
    db.add(off)
    await db.flush()
    return off
...
async def test_check_capabilities_satisfaction_r437(db):
    ...
    assert await svc.check_capabilities(
...
async def test_check_capabilities_scoping_r437(db):
    ...
    assert _codes(await svc.check_capabilities(
        org.id, [{"capability": "image_generation"}])) == ["CAPABILITY_UNSATISFIED"]
