...
async def _auth(c):
    r = await c.post(
        "/api/v1/auth/register", json={"email": _e(), "password": "Test123!", "display_name": "L55"}
    )
...
async def test_project_update_draft_not_owner(c):
    ...
    r3 = await c.put(
        f"/api/v1/orgs/{oid}/projects/{pid}/submissions/{subid}", json={"items": []}, headers=h2
    )
...
async def test_skill_exercise_update_wrong_org_check(c):
    ...
    r2 = await c.post(
        ...
        json={"category_id": cid, "name": "WOS2", "description": "D"},
        headers=h,
    )
    sid = r2.json()["data"]["id"]
    r3 = await c.post(
...
async def test_main_postgres_warn_dev():
    ...
    with (
        ...
        patch("app.main.redis_pool") as rp,
...
async def test_eval_execute_project_not_found():
    ...
    db.get = AsyncMock(
        side_effect=lambda cls, pk: mock_sub if "sub" in str(cls.__name__).lower() else None
    )
    db.flush = AsyncMock()
...
def test_eval_get_usage_with_existing_usage():
    ...
    db.execute = AsyncMock(
        return_value=MagicMock(scalar_one_or_none=MagicMock(return_value=mock_usage))
    )
    mock_org = MagicMock()
    ...
    svc = EvaluationService(db)
    result = (
        asyncio.get_event_loop_policy().new_event_loop().run_until_complete(svc.get_usage("org1"))
    )
...
async def test_portfolio_create_submission_not_found(db):
    ...
    with pytest.raises(AppError, match="not found"):
        await svc.create_item(
            u.id, "Bad", None, "nonexistent-sub", None, None, None, "public", False
        )
...
async def test_portfolio_create_submission_not_approved(db):
    ...
    with pytest.raises(AppError, match="approved"):
        await port_svc.create_item(
            u.id, "NotApproved", None, sub.id, None, None, None, "public", False
        )
...
async def test_project_set_skills_replaces(db):
    ...
    s1 = await skill_svc.create_skill(
        org.id, cat.id, "S1", None, "D", None, "beginner", None, None, None, u.id
    )
    s2 = await skill_svc.create_skill(
        org.id, cat.id, "S2", None, "D", None, "beginner", None, None, None, u.id
    )
    await db.flush()
    svc = ProjectService(db)
    proj = await svc.create_project(
...
async def test_skill_all_prereqs_completed(db):
    ...
    prereq = await svc.create_skill(
        org.id, cat.id, "PreReq2", None, "D", None, "beginner", None, None, None, u.id
    )
    ...
    adv = await svc.create_skill(
        org.id, cat.id, "Adv2", None, "D", None, "advanced", None, None, [prereq.id], u.id
    )
    await db.flush()
...
async def test_skill_progress_continue_counts(db):
    ...
    skill = await svc.create_skill(
        org.id, cat.id, "SPCSkill", None, "D", None, "beginner", None, None, None, u.id
    )
    ex1 = await svc.create_exercise(
        org.id,
        skill.id,
        "E1",
        "D",
