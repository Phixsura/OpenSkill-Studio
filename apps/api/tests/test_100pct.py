...
async def _u(db, role=UserRole.STUDENT):
    u = User(email=f"100-{uuid.uuid4().hex[:8]}@test.com", password_hash=hash_password("Test123!"),
             display_name="Hund", role=role, status=UserStatus.ACTIVE)
    db.add(u)
    await db.flush()
    return u
...
async def test_project_file_operations(db):
    ...
    proj = await svc.create_project(org.id, "FProj", None, "D", "I", "beginner", 100,
    ...
    with patch("app.core.storage.get_s3_client", return_value=fake_s3()):
        item = await svc.upload_file(sub.id, deliv.id, "test.py", b"print('hello')", "text/x-python", u.id)
...
async def test_project_extension_check(db):
    ...
    proj = await svc.create_project(
        org.id, "ExtProj", None, "D", "I", "beginner", 100,
    ...
    u2 = await _u(db)
    # Grant extension
    await svc.grant_extension(proj.id, u2.id, datetime.now(UTC) + timedelta(days=7), "Medical", u.id)
    await db.flush()
...
async def test_eval_trigger_with_mock_llm(db):
    ...
    proj = await proj_svc.create_project(
        org.id, "EvalProj", None, "D", "I", "beginner", 100,
        [{"criterion": "Quality", "max_score": 50}, {"criterion": "Design", "max_score": 50}],
        None, None, 0, 0, None, u.id)
    ...
    mock_response = LLMResponse(
        ...
        input_tokens=500, output_tokens=200, model="claude-sonnet-5", provider="anthropic",
    )
...
async def test_eval_trigger_llm_parse_failure(db):
    ...
    proj_svc = ProjectService(db)
    proj = await proj_svc.create_project(org.id, "EFProj", None, "D", "I", "beginner", 100,
                                          [{"criterion": "Q", "max_score": 100}], None, None, 0, 0, None, u.id)
    sub = await proj_svc.create_submission(org.id, proj.id, u.id)
    await proj_svc.submit_draft(sub.id, u.id)
    await db.flush()
    ...
    mock_response = LLMResponse(content="not json at all", input_tokens=100, output_tokens=50,
...
async def test_portfolio_badge_toggle(db):
    ...
    skill = await svc_s.create_skill(org.id, cat.id, "BS", None, "D", None, "beginner", None, None, None, u.id)
    await db.flush()
...
async def test_skill_progress_complete(db):
    ...
    skill = await svc.create_skill(org.id, cat.id, "ProgSkill", None, "D", None, "beginner", None, None, None, u.id)
    ex = await svc.create_exercise(org.id, skill.id, "ProgEx", "D", "multiple_choice",
                                    {"correct": ["a"], "options": [{"id": "a", "text": "R"}]}, 100, u.id)
    await db.flush()
...
async def test_rate_limit_dependency_denied():
    ...
    with patch("app.core.rate_limit.check_rate_limit", new_callable=AsyncMock, return_value=(True, 0)):
        await checker(mock_request)

    # Second call should deny
    from fastapi import HTTPException
    with patch("app.core.rate_limit.check_rate_limit", new_callable=AsyncMock, return_value=(False, 0)):
