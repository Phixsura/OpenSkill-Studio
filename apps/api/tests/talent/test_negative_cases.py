...
class TestScoringNegative:
    ...
    def test_all_voided(self):
        from app.talent.services.scoring import compute_score_from_evidence
        ev = [{"status": "voided", "score_normalized": 0.9, "verification_level": "employer_verified", "confidence": 1.0, "occurred_at": datetime.now(UTC), "expires_at": None}]
        s, _, _ = compute_score_from_evidence(ev, None, datetime.now(UTC))
        assert s == 0.0

    def test_all_expired(self):
        from app.talent.services.scoring import compute_score_from_evidence
        ev = [{"status": "active", "score_normalized": 0.9, "verification_level": "employer_verified", "confidence": 1.0, "occurred_at": datetime.now(UTC), "expires_at": datetime.now(UTC) - timedelta(hours=1)}]
        s, _, _ = compute_score_from_evidence(ev, None, datetime.now(UTC))
        assert s == 0.0

    def test_none_score(self):
        from app.talent.services.scoring import compute_score_from_evidence
        ev = [{"status": "active", "score_normalized": None, "verification_level": "self_reported", "confidence": 1.0, "occurred_at": datetime.now(UTC), "expires_at": None}]
        s, _, _ = compute_score_from_evidence(ev, None, datetime.now(UTC))
        assert s > 0

    def test_zero_confidence(self):
        from app.talent.services.scoring import compute_score_from_evidence
        ev = [{"status": "active", "score_normalized": 1.0, "verification_level": "employer_verified", "confidence": 0.0, "occurred_at": datetime.now(UTC), "expires_at": None}]
        s, _, _ = compute_score_from_evidence(ev, None, datetime.now(UTC))
        assert s >= 0
    ...
    def test_level_zero_evidence(self):
        ...
        l, _ = determine_level(0.99, 0, None)
        assert l == 0  # needs min_evidence

    def test_level_custom_defs_invalid(self):
        from app.talent.services.scoring import determine_level
        l, _ = determine_level(0.5, 5, {"invalid": "data"})
        assert isinstance(l, int)

    def test_negative_score_normalized(self):
        from app.talent.services.scoring import compute_score_from_evidence
        ev = [{"status": "active", "score_normalized": -10.0, "verification_level": "self_reported", "confidence": 1.0, "occurred_at": datetime.now(UTC), "expires_at": None}]
        s, _, _ = compute_score_from_evidence(ev, None, datetime.now(UTC))
        assert isinstance(s, float)

    def test_huge_score_normalized(self):
        from app.talent.services.scoring import compute_score_from_evidence
        ev = [{"status": "active", "score_normalized": 999.0, "verification_level": "employer_verified", "confidence": 1.0, "occurred_at": datetime.now(UTC), "expires_at": None}]
        s, _, _ = compute_score_from_evidence(ev, None, datetime.now(UTC))
        assert isinstance(s, float)
    ...
    def test_missing_verification_level(self):
        from app.talent.services.scoring import compute_score_from_evidence
        ev = [{"status": "active", "score_normalized": 0.8, "confidence": 1.0, "occurred_at": datetime.now(UTC), "expires_at": None}]
    ...
    def test_superseded_status(self):
        from app.talent.services.scoring import compute_score_from_evidence
        ev = [{"status": "superseded", "score_normalized": 0.9, "verification_level": "employer_verified", "confidence": 1.0, "occurred_at": datetime.now(UTC), "expires_at": None}]
        s, _, _ = compute_score_from_evidence(ev, None, datetime.now(UTC))
        assert s == 0.0
    ...
    def test_simulation_empty_current(self):
        ...
        r = simulate_score_change([], {"status": "active", "score_normalized": 0.8, "verification_level": "instructor_verified", "confidence": 1.0, "occurred_at": datetime.now(UTC), "expires_at": None})
    ...
    def test_auto_evidence_unknown_trigger(self):
        ...
        assert build_auto_evidence(user_id="u", capability_id="c", trigger_type="unknown", source_type="x", source_id="y") is None
    ...
    def test_mapping_confidence_zero_weight(self):
        ...
        c = compute_mapping_confidence(contribution_weight=0.0, evidence_type="self_declared", has_assessment=False, evidence_count=0)
        assert c >= 0
...
class TestPassportNegative:
    ...
    def test_html_xss_in_name(self):
        from app.talent.services.passport_intelligence import generate_passport_html
        html = generate_passport_html({"capabilities": [{"capability_name": "<script>alert(1)</script>", "level": 1, "level_label": "x", "score": 0.5, "evidence_count": 1}]})
    ...
    def test_revision_single(self):
        from app.talent.services.passport_intelligence import compute_revision_summary
        r = compute_revision_summary([{"id": "s1", "issued_at": "2026-01-01", "payload": {"capabilities": []}}])
        assert len(r) == 1

    def test_completeness_empty(self):
        from app.talent.services.profile_completeness import compute_profile_completeness
        r = compute_profile_completeness(passport=None, evidence_count=0, credential_count=0, has_verified_evidence=False)
        assert r.score == 0.0

    def test_completeness_with_data(self):
        from app.talent.services.profile_completeness import compute_profile_completeness
        r = compute_profile_completeness(passport={"discoverable": True, "availability_status": "open", "preferred_opportunity_types": ["job"]}, evidence_count=10, credential_count=2, has_verified_evidence=True)
        assert r.score > 0
    ...
    def test_governance_invalid_action(self):
        ...
        errors = validate_governance_request(action="invalid", capability_name="X", justification="Some reason here")
        assert len(errors) > 0

    def test_governance_short_justification(self):
        from app.talent.services.skill_intelligence import validate_governance_request
        errors = validate_governance_request(action="create", capability_name="X", justification="short")
        assert len(errors) > 0
    ...
    def test_duplicate_candidates_unique(self):
        from app.talent.services.skill_intelligence import find_duplicate_candidates
        caps = [{"id": "1", "canonical_name": "Python"}, {"id": "2", "canonical_name": "JavaScript"}]
...
class TestApplicationNegative:
    ...
    def test_reference_bad_email(self):
        from app.talent.services.application_intelligence import validate_reference
        errors = validate_reference({"referee_name": "John", "referee_email": "not-email", "referee_relationship": "Manager"})
        assert len(errors) > 0
    ...
    def test_onboarding_invalid_task_type(self):
        ...
        t = svc.create_template(name="T", org_id="o", tasks=[{"task_type": "invalid", "title": "X"}])
    ...
    def test_messaging_invalid_type(self):
        ...
        with pytest.raises(ValueError):
            svc.create_message(message_id="m1", application_id="a1", sender_id="u1", sender_role="candidate", message_type="invalid", content="Hi")

    def test_messaging_empty_content(self):
        from app.talent.services.messaging import MessagingService
        svc = MessagingService()
        with pytest.raises(ValueError):
            svc.create_message(message_id="m1", application_id="a1", sender_id="u1", sender_role="candidate", content="")
