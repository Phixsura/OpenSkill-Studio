...
def test_evidence_verification_levels_complete():
    ...
    required = {"employer_verified", "client_verified", "assessment_verified",
                "instructor_verified", "peer_verified", "system_observed", "self_reported"}
...
def test_intelligence_api_has_endpoints():
    from app.talent.api import intelligence
    from app.talent.api.intelligence import router as intel_router; paths = [r.path for r in intel_router.routes]
...
def test_outreach_router_registered():
    ...
    from app.talent.api.intelligence import router as intel_router; paths = [r.path for r in intel_router.routes]
    paths = [r.path for r in router.routes]; assert len(paths) >= 3
