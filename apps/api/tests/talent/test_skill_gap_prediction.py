...
class TestForecastSupply:
    def test_supply_growth(self):
        f = svc.forecast_supply(
            capability_id="c1", capability_name="AI",
            current_supply=50, learners_below_threshold=30,
    ...
    def test_zero_learners(self):
        f = svc.forecast_supply(
            capability_id="c1", capability_name="AI",
            current_supply=50, learners_below_threshold=0,
            avg_months_to_qualify=6,
        )
        assert f.estimated_graduates_3m == 0
...
class TestTimeToClsoure:
    def test_feasible_closure(self):
        e = svc.estimate_time_to_closure(
            capability_id="c1", capability_name="AI",
            current_gap=30, monthly_supply_growth=5,
    ...
    def test_infeasible_gap_growing(self):
        e = svc.estimate_time_to_closure(
            capability_id="c1", capability_name="AI",
            current_gap=30, monthly_supply_growth=2,
...
class TestBuildForecast:
    def test_critical_urgency(self):
        f = svc.build_forecast(
            ...
            current_demand=100, current_supply=40,
            growth_rate_90d=0.5, supply_growth_monthly=1,
        )
    ...
    def test_low_urgency_stable(self):
        f = svc.build_forecast(
            capability_id="c2", capability_name="Typing",
            current_demand=10, current_supply=50,
            growth_rate_90d=0.0, supply_growth_monthly=2,
        )
    ...
    def test_medium_urgency(self):
        f = svc.build_forecast(
            capability_id="c3", capability_name="ML",
            current_demand=50, current_supply=30,
            growth_rate_90d=0.1, supply_growth_monthly=1,
        )
