...
# 72 — career page is public (no auth gate) so it queries the DB directly.
# Under the noop-lifespan test fixture with no Postgres, the connection
# attempt hangs. Verify the route is registered instead.
def test_career_page_route_registered():
    from app.talent.api.employers import router

    paths = [r.path for r in router.routes]
    assert any("career-page" in p for p in paths)
