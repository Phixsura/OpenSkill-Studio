...
async def main():
    ...
    async with httpx.AsyncClient(base_url=API, timeout=30, transport=transport) as c:
        ...
        r1 = await c.post("/auth/register", json={"email": f"e2e-admin-{uid()}@test.com", "password": "TestPass123!", "display_name": "Admin"})
        check("Register admin", r1.status_code == 201, f"{r1.status_code}: {r1.text[:200]}")
        h1 = {"Authorization": f"Bearer {r1.json()['access_token']}"}

        r2 = await c.post("/auth/register", json={"email": f"e2e-user-{uid()}@test.com", "password": "TestPass123!", "display_name": "User"})
        ...
        cat = (await c.post(f"/orgs/{o1}/categories", json={"name": f"Cat-{uid()}"}, headers=h1)).json()["data"]["id"]
        check("Create category", bool(cat))

        s1 = (await c.post(f"/orgs/{o1}/skills", json={
            "name": "Prompt Engineering", "description": "Learn prompt engineering basics",
            "difficulty": "beginner", "category_id": cat,
        }, headers=h1)).json()["data"]["id"]
        check("Create skill 1", bool(s1))

        s2 = (await c.post(f"/orgs/{o1}/skills", json={
            "name": "Image Generation", "description": "AI image gen techniques",
            "difficulty": "intermediate", "category_id": cat,
        }, headers=h1)).json()["data"]["id"]
        check("Create skill 2", bool(s2))

        ex = await c.post(f"/orgs/{o1}/skills/{s1}/exercises", json={
            "title": "Write a Prompt", "description": "Write a basic prompt",
            "type": "text_answer", "config": {}, "max_score": 100,
        }, headers=h1)
        check("Create exercise", ex.status_code == 201, f"{ex.status_code}")

        tmpl = (await c.post(f"/orgs/{o1}/project-templates", json={
            "name": "Hero Shot", "description": "Create a hero product shot",
        ...
        # Add contents
        check("Add skill 1", (await c.post(f"/orgs/{o1}/packs/{pack_id}/skills", json={"skill_id": s1, "sort_order": 0}, headers=h1)).status_code == 201)
        check("Add skill 2", (await c.post(f"/orgs/{o1}/packs/{pack_id}/skills", json={"skill_id": s2, "sort_order": 1}, headers=h1)).status_code == 201)
        check("Add template", (await c.post(f"/orgs/{o1}/packs/{pack_id}/templates", json={"template_id": tmpl}, headers=h1)).status_code == 201)
        ...
        rel = await c.post(f"/orgs/{o1}/packs/{pack_id}/releases", json={"version": "1.0.0", "changelog": "Initial release"}, headers=h1)
        ...
        check("Registry search", sr.status_code == 200 and len(sr.json()["data"]) >= 1, f"found {len(sr.json().get('data', []))}")
        ...
        check("Consumer org has skills>=2", len(o2_skills.json()["data"]) >= 2, str(len(o2_skills.json()["data"])))
        ...
        s3 = (await c.post(f"/orgs/{o1}/skills", json={
            "name": "Advanced Composition", "description": "Advanced techniques",
            "difficulty": "advanced", "category_id": cat,
        }, headers=h1)).json()["data"]["id"]
        await c.post(f"/orgs/{o1}/packs/{pack_id}/skills", json={"skill_id": s3, "sort_order": 2}, headers=h1)
        rel2 = await c.post(f"/orgs/{o1}/packs/{pack_id}/releases", json={"version": "1.1.0", "changelog": "Added advanced skill"}, headers=h1)
        ...
        # Import into org3
        r3 = await c.post("/auth/register", json={"email": f"e2e-imp-{uid()}@test.com", "password": "TestPass123!", "display_name": "Imp"})
        h3 = {"Authorization": f"Bearer {r3.json()['access_token']}"}
        o3 = (await c.post("/orgs", json={"name": f"Imp-{uid()}"}, headers=h3)).json()["data"]["id"]

        imp = await c.post(f"/orgs/{o3}/packs/import", files={"file": ("pack.zip", exp.content, "application/zip")}, headers=h3)
        check("Import pack", imp.status_code == 201, f"{imp.status_code}: {imp.text[:300]}")
        check("Imported name matches", imp.json()["data"]["pack"]["name"].startswith("AI Photo Pack"))

        # ═══ 10. Learning Paths ═══
        print("\n📋 Phase 9: Learning Paths")
        o2_sk = [s["id"] for s in (await c.get(f"/orgs/{o2}/skills", headers=h2)).json()["data"][:2]]

        lp = await c.post(f"/orgs/{o2}/paths", json={"name": "AI Creator Path", "description": "Full track", "estimated_minutes": 480}, headers=h2)
        check("Create path", lp.status_code == 201, f"{lp.status_code}")
        path_id = lp.json()["data"]["id"]

        check("Add section", (await c.post(f"/orgs/{o2}/paths/{path_id}/items", json={"item_type": "section", "section_title": "Fundamentals", "sort_order": 0}, headers=h2)).status_code == 201)

        if o2_sk:
            check("Add skill item", (await c.post(f"/orgs/{o2}/paths/{path_id}/items", json={"item_type": "skill", "skill_id": o2_sk[0], "sort_order": 1}, headers=h2)).status_code == 201)
        if len(o2_sk) > 1:
            check("Add optional item", (await c.post(f"/orgs/{o2}/paths/{path_id}/items", json={"item_type": "skill", "skill_id": o2_sk[1], "sort_order": 2, "required": False}, headers=h2)).status_code == 201)

        items = await c.get(f"/orgs/{o2}/paths/{path_id}/items", headers=h2)
        check("List items>=2", len(items.json()["data"]) >= 2)

        check("Publish path", (await c.put(f"/orgs/{o2}/paths/{path_id}", json={"status": "published"}, headers=h2)).status_code == 200)
        ...
        asgn = await c.post(f"/orgs/{o2}/cohorts/{coh_id}/paths", json={"path_id": path_id}, headers=h2)
        check("Assign path to cohort", asgn.status_code == 201, f"{asgn.status_code}: {asgn.text[:200]}")

        cp = await c.get(f"/orgs/{o2}/cohorts/{coh_id}/paths", headers=h2)
        check("List cohort paths>=1", len(cp.json()["data"]) >= 1)

        check("Unassign path", (await c.delete(f"/orgs/{o2}/cohorts/{coh_id}/paths/{path_id}", headers=h2)).status_code == 204)

        # ═══ 12. Security ═══
        print("\n🔒 Phase 11: Cross-org Security")
        check("Cross-org pack 404", (await c.get(f"/orgs/{o2}/packs/{pack_id}", headers=h2)).status_code == 404)
        check("Cross-org publish 403/404", (await c.post(f"/orgs/{o2}/packs/{pack_id}/releases", json={"version": "9.9.9"}, headers=h2)).status_code in (403, 404))
        check("Cross-org install 404", (await c.get(f"/orgs/{o1}/installations/{inst_id}", headers=h1)).status_code == 404)
        check("Cross-org path 404", (await c.get(f"/orgs/{o1}/paths/{path_id}", headers=h1)).status_code == 404)

        # ═══ 13. Cleanup ═══
        print("\n🗑️  Phase 12: Remove installation")
        check("Remove installation", (await c.delete(f"/orgs/{o2}/installations/{inst_id}", headers=h2)).status_code == 204)
