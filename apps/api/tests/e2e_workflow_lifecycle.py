...
async def main() -> bool:  # noqa: PLR0915
    ...
    async with httpx.AsyncClient(base_url=API, timeout=30, transport=transport) as c:
        ...
        o1 = (await c.post("/orgs", json={"name": f"WfPub-{uid()}"}, headers=h1)).json()["data"][
            "id"
        ]
        o2 = (await c.post("/orgs", json={"name": f"WfCon-{uid()}"}, headers=h2)).json()["data"][
            "id"
        ]
        ...
        rd = await c.put(
            f"/orgs/{o1}/workflow-packs/{pack_id}/definition",
            json={"definition": _definition()},
            headers=h1,
        )
        check("Set definition", rd.status_code == 200, f"{rd.status_code}: {rd.text[:300]}")
        check(
            "Capability tags derived", rd.json()["data"]["capability_tags"] == ["image_generation"]
        )

        rv = await c.post(
            f"/orgs/{o1}/workflow-packs/validate", json={"definition": _definition()}, headers=h1
        )
        check("Dry-run validate ok", rv.status_code == 200 and rv.json()["data"]["valid"])

        rr = await c.post(
            f"/orgs/{o1}/workflow-packs/{pack_id}/releases",
            json={
                "version": "1.0.0",
        ...
        check(
            "Approve → public",
            ra.status_code == 200 and ra.json()["data"]["visibility"] == "public",
        )
        ...
        ri_fail = await c.post(
            f"/orgs/{o2}/workflow-installations",
            json={"pack_id": pack_id, "version": "1.0.0"},
            headers=h2,
        )
        check(
            "Install blocked without offering",
            ri_fail.status_code == 422
        ...
        rc = await c.post(
            f"/orgs/{o2}/provider-connections",
            json={"adapter_id": mock_id, "name": "Mock Provider"},
            headers=h2,
        )
        ...
        check(
            "Install succeeds with offering",
            ri.status_code == 201,
        ...
        check(
            "Binding suggestion created",
            rb.status_code == 200 and len(rb.json()["data"]) == 1,
        ...
        check(
            "Run suspends at review gate",
            run_data is not None and run_data["status"] == "waiting_review",
        ...
        check(
            ...
            str(gen.get("output", {}).get("result", "")).startswith("mock-asset-"),
        )
        ...
        review = next(
            (
                rv
                for rv in rrev.json()["data"]
                if any(s["id"] == rv["step_run_id"] for s in run_data["step_runs"])
            ),
            None,
        )
        ...
        rdec = await c.post(
            f"/orgs/{o2}/step-reviews/{review['id']}/decide",
        ...
        check(
            "Double-decide → 409",
            rdup.status_code == 409 and rdup.json()["error"]["code"] == "WF_REVIEW_ALREADY_DECIDED",
        )
        ...
        check(
            "Run output 'final' is a real value",
            isinstance(final_output, str) and bool(final_output),
        ...
        rprof = await c.post(
            f"/orgs/{o2}/requirement-profiles",
            json={
        ...
        check(
            ...
            rprof.status_code == 201,
            f"{rprof.status_code}: {rprof.text[:200]}",
        )
        ...
        rm = await c.post(
            f"/orgs/{o2}/match",
            json={
                "requirement_profile_id": prof_id,
        ...
        check(
            "Engine + config versions recorded",
            match_data["engine_version"] == "1.0.0" and match_data["config_version"] >= 1,
        )

        rdraft = await c.post(
            f"/orgs/{o2}/drafts/production-solution", json={"profile_id": prof_id}, headers=h2
        )
        check(
            "Production draft composed",
            rdraft.status_code == 201,
            f"{rdraft.status_code}: {rdraft.text[:300]}",
        )
        ...
        rldraft = await c.post(
            f"/orgs/{o2}/drafts/learning-path", json={"profile_id": lprof_id}, headers=h2
        )
        check(
            "Learning draft composed",
            rldraft.status_code == 201,
            f"{rldraft.status_code}: {rldraft.text[:300]}",
        )
        ldraft_id = rldraft.json()["data"]["id"]

        rconf = await c.post(f"/orgs/{o2}/drafts/{ldraft_id}/confirm", headers=h2)
        check(
            "Confirm learning draft",
            rconf.status_code == 200,
            f"{rconf.status_code}: {rconf.text[:300]}",
        )
        ...
        check(
            "Create project", rproj.status_code == 201, f"{rproj.status_code}: {rproj.text[:200]}"
        )
        ...
        check(
            "Shortlist generated",
            rshort.status_code == 200,
            f"{rshort.status_code}: {rshort.text[:200]}",
        )

        roffer = await c.post(
            f"/orgs/{o2}/creator-assignments",
            json={
                "project_id": proj_id,
                "user_id": u2_id,
            },
        ...
        check(
            "Offer assignment (human action)",
            roffer.status_code == 201,
            f"{roffer.status_code}: {roffer.text[:200]}",
        )
        assign_id = roffer.json()["data"]["id"]
        check("Assignment starts as offer", roffer.json()["data"]["status"] == "offered")

        raccept = await c.post(
            f"/orgs/{o2}/creator-assignments/{assign_id}/respond", json={"accept": True}, headers=h2
        )
        check(
            "Creator accepts",
            raccept.status_code == 200 and raccept.json()["data"]["status"] == "accepted",
        )
