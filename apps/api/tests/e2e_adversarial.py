...
async def main():
    async with httpx.AsyncClient(base_url=API, timeout=30, trust_env=False) as c:
        ...
        v_sub = (await c.post(f"/orgs/{v_org}/projects/{v_proj}/submissions", headers=vh)).json()[
            "data"
        ]["id"]
        v_pack = (
            await c.post(
        ...
        for verb, path, body in [
            ...
            (
                "POST",
                f"/orgs/{v_org}/submissions/{v_sub}/reviews",
                {"status": "approved", "score": 100, "feedback": "pwn"},
            ),
            ("GET", f"/orgs/{v_org}/packs/{v_pack}", None),
            ("POST", f"/orgs/{v_org}/packs/{v_pack}/skills", {"skill_id": v_skill}),
        ...
        for verb, path, body in [
            ...
            (
                "POST",
                f"/orgs/{a_org}/submissions/{v_sub}/reviews",
            ...
            (
                "POST",
                f"/orgs/{a_org}/projects/{v_proj}/client-links",
                {"role": "approver", "expires_at": _exp()},
            ),
        ]:
            r = await c.request(verb, path, json=body, headers=ah)
        ...
        for verb, path, body in [
            ...
            (
                "POST",
                f"/platform/tenants/{v_tenant or 'x'}/credits/adjust",
        ...
        r = await c.post(
            f"/orgs/{a_org}/members", json={"user_id": v_uid, "role": "owner"}, headers=vh
        )
        ...
        wrong_key = pyjwt.encode(
            {"sub": v_uid, "type": "access", "role": "admin"},
            key="guessed-secret",
            algorithm="HS256",
        )
        ...
        if link.status_code == 201:
            ...
            if gtok:
                ...
                check(
                    "guest token blocked on product API",
        ...
        r = await c.post(
            ...
            json={"name": "big", "description": "A" * 2_000_000},
            headers=ah,
        )
        ...
        for verb, path in [
            ...
            check(
                f"anon {verb} {path.split('/')[1]}… → 401/403",
                r.status_code in (401, 403),
                f"got {r.status_code}",
            )
    ...
    async with httpx.AsyncClient(base_url=API, timeout=30, trust_env=False) as c:
        ...
        r = await c.post(
            "/billing/webhooks/mock", content=evil, headers={"Content-Type": "application/json"}
        )
        check("unsigned mock webhook → 401", r.status_code == 401, f"got {r.status_code}")
        r = await c.post(
            "/billing/webhooks/mock",
            content=evil,
            headers={"Content-Type": "application/json", "X-Mock-Signature": "f" * 64},
        )
        ...
        r = await c.post(
            ...
            headers=httpx.Headers(
                {b"Content-Type": b"application/json", b"X-Mock-Signature": b"\xff\xfe garbage"}
            ),
        )
        check(
            "non-ASCII signature header → 401 not 500", r.status_code == 401, f"got {r.status_code}"
        )
        r = await c.post(
            "/billing/webhooks/manual", content=evil, headers={"Content-Type": "application/json"}
        )
        check(
            "manual-provider webhook rejected", r.status_code in (401, 404), f"got {r.status_code}"
        )
        r = await c.post(
            "/billing/webhooks/doesnotexist",
            content=evil,
            headers={"Content-Type": "application/json"},
        )
        check(
            "unknown provider webhook rejected", r.status_code in (401, 404), f"got {r.status_code}"
        )
        r = await c.post(
        ...
        check(
            "100KB unsigned stripe body → 4xx not 5xx",
    ...
    async with httpx.AsyncClient(base_url=API, timeout=60, trust_env=False) as c:
        ...
        r = await c.post(
            f"/orgs/{v_org}/workflow-packs",
            json={"name": f"VWF {uuid.uuid4().hex[:6]}"},
            headers=vh,
        )
        v_wf = r.json()["data"]["id"]
        r = await c.put(
            f"/orgs/{v_org}/workflow-packs/{v_wf}/definition",
            json={"definition": wf_def},
            headers=vh,
        )
        check("victim wf definition accepted", r.status_code == 200, r.text[:120])
        await c.post(
            f"/orgs/{v_org}/workflow-packs/{v_wf}/releases", json={"version": "1.0.0"}, headers=vh
        )
        ...
        async with AsyncSessionLocal() as s:
            listing = MarketplaceListing(
                ...
                product_id=v_wf,
                seller_org_id=v_org,
                seller_tenant_id=v_tenant or v_org,
                offer_type="paid",
                price_minor=21494,
                currency="USD",
        ...
        check(
            "paid pack preview redacted",
            r.status_code == 200 and r.json()["data"]["definition"].get("redacted") is True,
            r.text[:120],
        )
        ...
        r = await c.post(
            f"/orgs/{a_org}/workflow-installations", json={"pack_id": v_wf}, headers=ah
        )
        ...
        if r.status_code == 402:
            ...
            check("price tamper ignored (server-side price)", amt == 21494, f"amount {amt}")
        else:
            check(
                "purchase attempt clean 4xx",
                400 <= r.status_code < 500,
        ...
        for verb, path in [
            ...
            denied(
                r, f"{verb} {path.split('/')[2]} money op as student", allow=(401, 403, 404, 422)
            )
        ...
        for path in [
            ...
            r = await c.get(path)
            check(
                f"anon {path.split(v_pack)[-1] or '/detail'} of PRIVATE pack → 404",
                r.status_code == 404,
                f"got {r.status_code}",
            )
        r = await c.post(
        ...
        for path in [
            ...
            r = await c.get(path)
            check(
                f"anon draft wf {path.split(v_draft_wf)[-1] or '/detail'} → 404",
                r.status_code == 404,
                f"got {r.status_code}",
            )
        ...
        link_r = await c.post(
            f"/orgs/{v_org}/projects/{v_proj}/client-links",
            json={"role": "reviewer", "expires_at": _exp()},
            headers=vh,
        )
        ...
        r = await c.post(
            "/client-portal/guest-session", json={"token": raw_e, "email": "attacker@evil.com"}
        )
        check("email-bound link + wrong email → 401", r.status_code == 401, f"got {r.status_code}")
        ...
        check(
            "100-char password → clean (no bcrypt-72 500)",
        ...
        r = await c.post(
            "/auth/register",
            json={"email": dup_email, "password": "Adv3rs4ry!x", "display_name": "Dup Two"},
        )
        ...
        check(
            "case-variant duplicate email rejected",
        ...
        check(
            "control-char password → 4xx not 500",
            400 <= r.status_code < 500,
            f"got {r.status_code}",
        )
        r = await c.post(
        ...
        check("change-password wrong old → 4xx", 400 <= r.status_code < 500, f"got {r.status_code}")
        ...
        check(
            "content-type spoof (shell as PNG) → 422", r.status_code == 422, f"got {r.status_code}"
        )
        r = await c.post(
            f"/orgs/{v_org}/submissions/{v_sub}/files",
    ...
    async with httpx.AsyncClient(base_url=API, timeout=30, trust_env=False) as c:
        ...
        r = await c.post(
            ...
            json={"role": "student", "max_uses": 1, "expires_in_days": 7},
            headers=vh,
        )
        ...
        r = await c.post(
            f"/orgs/{v_org}/invite-links",
            json={"role": "owner", "max_uses": 1, "expires_in_days": 7},
            headers=j1h,
        )
    ...
    async with httpx.AsyncClient(base_url=API, timeout=30, trust_env=False) as c:
        ...
        check("imp token blocked on client portal", r.status_code == 403, f"got {r.status_code}")
        ...
        r = await c.get(f"/orgs/{v_org}/skills", headers=imp_h)
        check(
            "revoked grant kills the LIVE imp token", r.status_code == 401, f"got {r.status_code}"
        )
        r = await c.post(f"/platform/impersonation-grants/{grant_id}/token", headers=admin_h)
        ...
        r = await c.get(
            f"/platform/tenants/{v_tenant or 'x'}/exports/01FAKEEXPORT00000000000000", headers=ah
        )
        ...
        for verb, path in [
            (
                "GET",
                f"/orgs/{v_org}/requirement-profiles",
            ),
            ...
            denied(r, f"{verb} {path.split('/', 3)[-1][:40]}", allow=(401, 403, 404, 405, 422))

        # ═══ S. Review/discussion abuse ═══
        section("S. reviews + discussions")
        r = await c.post(
            f"/registry/packs/{v_pack}/reviews",
            json={"rating": 1, "title": "pwn", "body": "trash"},
            headers=ah,
        )
        denied(r, "review a PRIVATE (unlisted) pack", allow=(401, 403, 404, 409, 422))
        r = await c.post(f"/registry/packs/{v_pack}/discussions", json={"body": "spam"}, headers=ah)
        denied(r, "discuss a PRIVATE pack", allow=(401, 403, 404, 422))
        # anonymous writes
        r = await c.post(
            f"/registry/packs/{v_pack}/reviews", json={"rating": 5, "title": "x", "body": "y"}
        )
        check(
            "anon review write → 401/403/404",
        ...
        r = await c.put(
            f"/orgs/{a_org}/settings",
            json={"settings": {"ai_evaluation": {"enabled": True, "monthly_budget_usd": 999999}}},
            headers=ah,
        )
        denied(r, "reserved ai_evaluation key via generic settings", allow=(400, 403, 422))
        r = await c.put(f"/orgs/{v_org}/settings", json={"settings": {"theme": "dark"}}, headers=ah)
    ...
    async with httpx.AsyncClient(base_url=API, timeout=60, trust_env=False) as c:
        ...
        check(
            "8-way purchase idempotency race: ≤1 purchase id",
            len(ids) <= 1,
        ...
        no_500 = all(r.status_code < 500 for r in rs)
        check(
            "6-way max_uses=1 race: no 500s", no_500, f"codes={sorted(r.status_code for r in rs)}"
        )
        check("6-way max_uses=1 race: exactly 1 join", len(joins) == 1, f"{len(joins)} joins")

        # U3: portal final-accept ×6 parallel → exactly one 201
        r = await c.post(f"/orgs/{v_org}/projects/{v_proj}/submissions/{v_sub}/submit", headers=vh)
        check("victim submits draft (U3 setup)", r.status_code == 200, r.text[:120])
        await c.post(
            f"/orgs/{v_org}/projects/{v_proj}/client-shares",
            json={"submission_id": v_sub},
            headers=vh,
        )
        la = await c.post(
            f"/orgs/{v_org}/projects/{v_proj}/client-links",
        ...
        async def _final():
            return await c.post(
                f"/client-portal/projects/{v_proj}/final-accept",
                json={"submission_id": v_sub, "comment": "race"},
                headers=atok,
            )
        ...
        no_500 = all(r.status_code < 500 for r in rs)
        check(
            "6-way final-accept race: no 500s", no_500, f"codes={sorted(r.status_code for r in rs)}"
        )
        check(
            "6-way final-accept race: exactly 1 acceptance", len(oks) == 1, f"{len(oks)} accepted"
        )
        ...
        async def _mkorg(hh):
            return await c.post(
                "/orgs",
                json={"name": "Race Org X", "slug": slug, "description": "d"},
                headers=hh,
            )
        ...
        no_500 = all(r.status_code < 500 for r in rs)
        check(
            "6-way same-slug org race: no 500s",
            no_500,
            f"codes={sorted(r.status_code for r in rs)}",
        )
        check("6-way same-slug org race: exactly 1 winner", len(wins) == 1, f"{len(wins)} wins")
        ...
        for target in [
            ...
            r = await c.post(
                ...
                json={"url": target, "events": ["pack.published"]},
                headers=ah,
            )
        ...
        check(
            "public https webhook accepted (positive control)",
        ...
        check(
            "duplicate JSON keys: no 500, deterministic parse",
            r.status_code < 500,
            f"got {r.status_code}",
        )
        r = await c.post(
        ...
        check(
            "anon HEAD on protected route: no body leak",
        ...
        check(
            "forgot-password: existence not disclosed",
            f1.status_code == f2.status_code,
            f"{f1.status_code} vs {f2.status_code}",
        )
        ...
        r = await c.post(
            ...
            files={
                "file": ("bomb.zip", b"PK\x03\x04" + b"A" * (6 * 1024 * 1024), "application/zip")
            },
            headers=ah,
        )
