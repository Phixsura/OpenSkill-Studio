...
def apply_site(kind, node, idx, repl):
    if kind == "cmp":
        old = node.ops[idx]
        node.ops[idx] = repl()
        return lambda: node.ops.__setitem__(idx, old)
...
def run(path, funcs, test_cmd, limit=None, timeout=300, only_descs=None):
    ...
    st = subprocess.run(
        ["git", "status", "--short", str(p)], capture_output=True, text=True
    ).stdout.strip()
    if st:
    ...
    if base.returncode != 0:
        raise SystemExit(
            f"BASELINE RED for {path} — fix the tests before mutating (rc={base.returncode})"
        )
    ...
    try:
        for n, (kind, node, idx, repl, desc) in enumerate(sites):
            ...
            if rc == 0:
                ...
                killed += 1
                tag = "killed (timeout)" if rc == -1 else "killed"
    ...
    dt = time.time() - t0
    print(
        f"  DONE {killed} killed / {len(survived)} survived in {dt:.0f}s ({len(sites) / max(dt, 1):.2f}/s)"
    )
    return survived
...
if __name__ == "__main__":
    ...
    for t in cfg:
        s = run(
            ...
            t.get("limit"),
            t.get("timeout", 300),
            set(t["only_descs"]) if t.get("only_descs") else None,
        )
        if s:
            all_surv[f"{t['path']}:{','.join(t['funcs'])}"] = s
    print("\n═══ SURVIVORS ═══")
