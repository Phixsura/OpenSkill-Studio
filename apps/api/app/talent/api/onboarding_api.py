...
@router.post(
    "/orgs/{org_id}/onboarding-templates",
...
async def list_onboarding_templates(
    ...
    return CursorListResponse(
        data=[{"id": t.id, "name": t.name, "status": t.status} for t in items],
        meta=CursorMeta(next_cursor=nc, has_more=has_more),
    )


@router.post(
    "/placements/{placement_id}/onboarding",
    response_model=DataResponse[dict],
    status_code=201,
...
async def create_onboarding_checklist(
    ...
    cl = OnboardingChecklist(
        placement_id=placement_id, template_id=body.get("template_id"), tasks=body.get("tasks", [])
    )
    db.add(cl)
    await db.commit()
    await db.refresh(cl)
